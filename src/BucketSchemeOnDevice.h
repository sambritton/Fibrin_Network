...
struct NgbrFunc : public thrust::unary_function<Tuu, Tuu> {
	unsigned XSize;
	unsigned YSize;
	unsigned ZSize;
	...
	__device__ Tuu operator()(const Tuu &v) {
		unsigned relativeRank = thrust::get<1>(v) % 27;
		unsigned XYSize = XSize * YSize;
		unsigned zPos = thrust::get<0>(v) / XYSize;
		unsigned xyPos = thrust::get<0>(v) % XYSize;
		unsigned xPos = xyPos % XSize;
		unsigned yPos = xyPos / XSize;

		unsigned rankZ = relativeRank / 9;
		unsigned rankTmp = relativeRank % 9;
		unsigned rankY = rankTmp / 3;
		unsigned rankX = rankTmp % 3;
		...
		if (rankX == -1) {
			...
			if (xPos == 0) {
				return thrust::make_tuple(ULONG_MAX, thrust::get<1>(v));
			}
		}
		else if (rankX == 1) {
		...
		unsigned ngbrPos = (xPos + rankX) + (yPos + rankY) * XSize
			+ (zPos + rankZ) * XYSize;
